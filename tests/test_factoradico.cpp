...
int test_factoradico(){
    ...
    check("0! = 1", LargeNumber::comparar_numeros(Factoradico::factorial(0), LargeNumber(1)) == 0);
    check("5! = 120", LargeNumber::comparar_numeros(Factoradico::factorial(5), LargeNumber(120)) == 0); 
    
    // Número a factorádico
    auto f17 = Factoradico::NumeroAFactoradico(LargeNumber(17));
    check("17 fact: size=4", f17.size() == 4);
    check("17 fact: [0]=2", LargeNumber::comparar_numeros(f17[0], LargeNumber(2)) == 0);
    check("17 fact: [1]=2", LargeNumber::comparar_numeros(f17[1], LargeNumber(2)) == 0);
    check("17 fact: [2]=1", LargeNumber::comparar_numeros(f17[2], LargeNumber(1)) == 0);
    check("17 fact: [3]=0", LargeNumber::comparar_numeros(f17[3], LargeNumber(0)) == 0);

    // Factorádico a número
    std::vector<LargeNumber> f = {LargeNumber(2), LargeNumber(2), LargeNumber(1), LargeNumber(0)};
    check("[2, 2, 1, 0] = 17", LargeNumber::comparar_numeros(Factoradico::FactoradicoANumero(f), LargeNumber(17)) == 0);
